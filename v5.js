...
function schedulePreview(source,status,img,retryButton=null,delay=500){
  ...
  previewTimer=setTimeout(()=>{
    ...
    const req={source,status,img,retryButton,token};
    if(previewBusy){queuedPreview=req;status.textContent='Waiting for current MetaTrader preview to finish…';return;}
    runPreview(req);
  },delay);
}

async function runPreview(req){
  const {source,status,img,retryButton,token}=req;
  if(token!==activePreviewToken||detailFilePath()!==source)return;
  previewBusy=true;
  const kind=/\.(mq5|ex5)$/i.test(source)?'MT5':'MT4';
  const started=performance.now();
  if(retryButton)retryButton.disabled=true;
  status.textContent=`Rendering real ${kind} preview automatically…`; img.style.display='none';
  ...
  try{
    ...
    await previewLog('INFO','preview_success',{source,kind:result.kind||kind,image:result.image,cached:!!result.cached,terminal:result.terminal?.terminal,meta:result.meta||{},elapsedMs:Math.round(performance.now()-started),automatic:true});
  }catch(e){
    ...
    if(next&&next.token===activePreviewToken&&detailFilePath()===next.source)setTimeout(()=>runPreview(next),80);
  }
}
...
function injectPreviewCard(){
  ...
  section.innerHTML=`<div class="label">Real MetaTrader Preview</div>
    <div style="border:1px solid rgba(127,127,127,.22);border-radius:10px;padding:10px;margin-top:8px">
      <div class="muted" style="margin-bottom:9px">The real ${kind} preview renders automatically. Existing EX4/EX5 files are preferred; otherwise the source is staged inside MetaTrader and compiled there. Successful previews are cached.</div>
  ...
  schedulePreview(source,status,img,retry,500);
}
...
function enhanceSourceRemoval(){
  ...
  for(const row of host.querySelectorAll('.source')){
    ...
    btn.onclick=async e=>{
      ...
      btn.disabled=true;
      try{const db=await previewDb(); const result=await previewCmd(['remove-source','--db',db,'--source',path]);await previewLog('INFO','source_removed',{source:path,removedIndicators:result.removed_indicators});location.reload();}
...
async function archiveAndFresh(button,status){
  ...
  try{const db=await previewDb(); const result=await previewCmd(['archive-reset','--db',db]);status.textContent=`Archive created: ${result.archive}`;setTimeout(()=>location.reload(),900);}
...
async function enhanceSettings(){
  for(let i=0;i<50;i++){
    const brand=document.querySelector('.brand small'); if(brand)brand.textContent='0.5.3 • Evidence Engine v4 + Automatic MT4/MT5 Preview';
    ...
    if(panel&&!document.getElementById('v5MetaTraderSettings')){
      ...
      card.innerHTML=`<h3>MetaTrader Preview</h3><p class="muted">0.5.3 renders real MT4 and MT5 previews automatically. Preview jobs are serialized, matching EX4/EX5 binaries are preferred, and MetaEditor compilation uses a staged workspace with detailed compile diagnostics.</p><div id="v5TerminalStatus" class="status">Detecting MetaTrader…</div>
      ...
      const archiveBtn=card.querySelector('#v5ArchiveFresh'),archiveStatus=card.querySelector('#v5ArchiveStatus'); archiveBtn.onclick=()=>archiveAndFresh(archiveBtn,archiveStatus);
      try{const detected=await previewCmd(['detect']);const terms=detected.terminals||[];document.getElementById('v5TerminalStatus').innerHTML=terms.length?terms.map(t=>`<div>${esc(t.kind)} • ${esc(t.install_dir)}${t.data_dir?' • data mapped':' • data folder not mapped yet'}</div>`).join(''):'No MetaTrader installation detected yet.';}
      catch(e){document.getElementById('v5TerminalStatus').textContent=`Detection error: ${e.message}`;}
      try{const base=await appDataDir(),db=await join(base,'library.sqlite3');const m=await previewCmd(['memory-stats','--db',db]);document.getElementById('v5MemoryStatus').textContent=`${m.corrections} correction memories • ${m.verified} verified indicators • ${m.families} corrected families`;}
      catch(e){document.getElementById('v5MemoryStatus').textContent=`Memory status unavailable: ${e.message}`;}
      enhanceSourceRemoval(); return;
    }
    await sleep(100);
  }
}
