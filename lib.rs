...
fn start_preview_library(app: tauri::AppHandle, args: Vec<String>) -> Result<Value, String> {
    ...
    let db_supervisor=db_path.clone();
    let engine_supervisor=engine_path.clone();
    let args_supervisor=args.clone();
    std::thread::spawn(move || {
        ...
        loop {
            append_log(&db_supervisor,"INFO","preview_library_start",json!({
                "engine":engine_supervisor,
                "args":args_supervisor,
                "restart_count":restarts
            }),None);

            let mut cmd=ProcessCommand::new(&engine_supervisor);
            cmd.args(&args_supervisor)
                .env("PYTHONUTF8","1")
                .env("PYTHONIOENCODING","utf-8")
                .stdout(Stdio::piped())
                .stderr(Stdio::piped());
            ...
            let mut child=match cmd.spawn() {
                ...
                Err(e)=>{
                    final_code=-1;
                    append_log(&db_supervisor,"ERROR","preview_library_spawn_failed",json!({"error":e.to_string(),"restart_count":restarts}),None);
                    if restarts>=MAX_RESTARTS { break; }
                    restarts+=1;
                    append_log(&db_supervisor,"WARN","preview_library_restart",json!({"reason":"spawn_failed","restart_count":restarts}),None);
                    let _=app_supervisor.emit("preview-library-restart",json!({"reason":"spawn_failed","restart_count":restarts}));
            ...
            let out_thread=stdout.map(|stdout|{
                let app_out=app_supervisor.clone();
                let db_out=db_supervisor.clone();
            ...
            let err_thread=stderr.map(|stderr|{
                let app_err=app_supervisor.clone();
                let db_err=db_supervisor.clone();
                std::thread::spawn(move || {
                    for line in BufReader::new(stderr).lines().map_while(Result::ok) {
            ...
            loop {
                match child.try_wait() {
                    ...
                    Err(e)=>{
                        final_code=-1;
                        append_log(&db_supervisor,"ERROR","preview_library_poll_failed",json!({"error":e.to_string()}),None);
                ...
                if age>HEARTBEAT_TIMEOUT_SECS {
                    ...
                    final_code=-2;
                    append_log(&db_supervisor,"ERROR","preview_library_heartbeat_timeout",json!({
                        "age_seconds":age,
                        "restart_count":restarts
                    }),None);
                    kill_process_tree(&mut child);
            ...
            if restart_reason.is_none() && final_code==0 {
                append_log(&db_supervisor,"INFO","preview_library_done",json!({"code":0,"restarts":restarts}),None);
                break;
            }

            if restarts>=MAX_RESTARTS {
                append_log(&db_supervisor,"ERROR","preview_library_restart_cap",json!({
                    "code":final_code,
                    "restart_count":restarts,
                    "reason":restart_reason.unwrap_or("worker_exit")
                }),None);
                let _=app_supervisor.emit("preview-library-restart-cap",json!({
                    "code":final_code,"restart_count":restarts
                }));
                break;
            }
            ...
            append_log(&db_supervisor,"WARN","preview_library_restart",json!({
                "reason":reason,
                "code":final_code,
                "restart_count":restarts
            }),None);
            let _=app_supervisor.emit("preview-library-restart",json!({
                "reason":reason,
                "code":final_code,
                "restart_count":restarts
            }));
        ...
        PREVIEW_LIBRARY_RUNNING.store(false, Ordering::SeqCst);
        let _=app_supervisor.emit("preview-library-done",json!({"code":final_code,"restarts":restarts}));
    });

    Ok(json!({"started":true,"already_running":false,"engine":engine_path.to_string_lossy(),"heartbeat_timeout_seconds":120,"max_restarts":20}))
}
